...
# ── 파일별 그룹 검색: 기본 끔 ───────────────────────────────────────────
# 거대 문서(EnergyPlus 매뉴얼 8,940 청크)가 후보를 독점할까 걱정해서 만들었다.
# 그 독점은 실측에서 일어나지 않았다 — 최다 점유 문서는 연구실이 직접 쓴
# GreenLightPlus_총정리.docx 였다. eval 문항 40개로는 답이 갈린 문항이 3개뿐이라
# McNemar 정확검정 p=1.0 이었다. 문항을 104개로 늘려 재검정해도 결과는 같다 —
# 답 갈린 문항 5개(grouped 2승 3패), p=1.0. 표본을 2.6배 늘려도 이득도 손해도
# 증명되지 않았다 (자세한 계산은 README "eval 셋으로 절제 실험" 항목 참고).
#
# 그래도 기본을 끄는 이유는 통계적 우위가 아니라: 걱정했던 문제가 실측에서
# 없었던 이상, 증명 안 된 복잡성을 기본으로 켜둘 이유가 없다. 청크 수가 극단적인
# 문서가 새로 들어오거나 eval 문항이 더 늘면 다시 켜고 재본다.
#
# 재검정(2026-07-30, 청킹 손실 버그 수정·문맥 확장·하이브리드 검색 추가 이후 96문항
# 정제셋): Recall 81→83/96(+2), MRR 0.785→0.815, 답 갈린 문항 4개(grouped 3승
# 1패) — 방향은 긍정적이지만 McNemar 정확검정 p=0.625로 여전히 유의하지 않다
# (Codex 자문 결과도 동일 결론: 표본이 너무 작아 판별력이 없다, "OFF 유지, 단
# ON 전환 후보로 추가 검증할 가치는 있다"). 후보 독점이 실제로 일어나는 질문군
# (긴 강의자료·매뉴얼·회의록)과 단일 대문서 심층검색 질문군을 20~30개씩 따로
# 만들어 candidate recall 단위로 재보면 더 확실해질 것 — 지금은 보류.
GROUPED_ENABLED = False
GROUPS = 25
PER_FILE = 3

# ── 중복 제거: 기본 끔 ──────────────────────────────────────────────────
# 버전 파일(_v1.1, _v1.2)이 답변 근거 6칸의 절반을 먹는 것을 실제로 목격해서
# 만들었다. eval 문항 40개로 켜고/끈 차이를 재봤을 때는 답이 갈린 문항이 2개뿐이라
# McNemar 정확검정 p=0.5 — "해롭다"는 결론은 과신이었다. 문항을 104개로 늘려
# 재검정한 결과: 답 갈린 문항 6개, 전부 dedup 손해 방향(dedup 6패 0승) —
# McNemar 정확검정 p=0.0312 로 **유의하게 해롭다**. 자카드 유사도 임계값이 버전
# 파일(같은 문서)과 같은 주제의 다른 문서를 구조적으로 구분하지 못해, 서로 다른
# 근거를 우연히 걸러내는 쪽으로 작동한 것으로 보인다 (자세한 계산은 README 참고).
#
# 이제는 통계적 우위 부재가 아니라 **통계적으로 확인된 손해** 때문에 기본을 끈다.
# 켜려면 retrieve(dedup_threshold=0.85) 로 직접 지정 — 기본값을 바꾸지 말 것.
#
# 재검정(2026-07-30, 같은 파이프라인 변경 이후 96문항 정제셋): dedup on/off의
# Recall이 81/96로 완전히 같고 **답 갈린 문항이 0개**다 — 예전의 "6패 0승,
# p=0.0312 유의하게 해롭다"가 지금 파이프라인에서는 재현되지 않는다(청킹 수정으로
# 자카드 유사도가 서로 다른 근거를 오인해 거르던 상황 자체가 줄어든 것으로 보임).
# MRR은 0.785→0.795로 소폭 올랐지만 이미 맞힌 문항의 순위 변화일 뿐이라 recall
# 판단과 무관하다. Codex 자문: "해롭지 않음"을 확인한 것이지 "이롭다"를 증명한
# 게 아니다 — 불일치 0개는 dedup이 개입할 기회 자체가 줄어든 결과일 수도 있고,
# 알고리즘이 근사 중복 판단이라는 구조적 위험은 여전하다. 기본값은 계속 OFF로
# 둔다 — 이번엔 "확인된 손해"가 아니라 "이롭다는 증거 부족"이 이유로 바뀐 것.
DEDUP_ENABLED = False
DEDUP_THRESHOLD = 0.85

# ── 문맥 확장(small-to-big): 기본 켬 ────────────────────────────────────
# 리랭커가 고른 청크 하나만으로는 문맥이 부족한 경우가 있다 (예: 회의록에서
# 담당자 이름은 앞 섹션에, 세부 내용만 청크에 있는 등). Codex 자문(2026-07-30)
# 결과를 따라 가장 보수적인 범위로 구현했다 — top-1 결과에만 앞뒤 ±1개를 붙인다
# (전체 top_k 각각에 부모 섹션 전체를 붙이는 것은 프롬프트 비용·잡음이 커서 배제).
#
# eval(96~104문항, ground-truth 정제 전후 둘 다)로는 Recall@6·MRR 에 유의한 차이가
# 없었다 — 이 eval은 "정답 파일이 검색되는가"만 재는데, 확장은 이미 뽑힌 top-1
# 뒤에 이웃을 끼워 넣을 뿐 어떤 파일이 뽑히는지는 안 바꾸므로 애초에 이 지표로는
# 효과를 볼 수 없는 구조다 (MRR 이 0.770→0.751 로 떨어진 것도 순위 계산의
# 부작용이지 답변이 나빠진 게 아니었다).
#
# 그래서 recall 대신 실제 생성 답변 5개를 확장 켬/끔으로 나란히 비교했다. 4개는
# 차이가 없었고, 1개("태백 대형팜과 남부터미널 드론 모델")는 뚜렷하게 나아졌다 —
# 확장 없이는 "DJI NEO"만 답하고 전환 사실을 각주처럼 흘렸는데, 확장을 켜니
# "DJI NEO → 한계로 Bee35 전환"을 두 항목으로 명확히 구분해 답했다. top-1 청크가
# 문장 중간에 끊겨서 뒷내용이 다음 청크에 있던 경우였다 — 정확히 small-to-big이
# 노리는 상황이다. 나빠진 사례는 없었다. 통계적 유의성은 없지만(그런 지표가 없어서
# 못 낸 것), 손해 없이 이득 사례가 실측으로 확인돼 기본을 켠다.
...
def dedup(hits: list[Hit], scores: list[float], top_k: int,
    ...
    ⚠️ **기본으로 꺼져 있다** (DEDUP_ENABLED=False). eval 문항을 104개로 늘려 재검정한
...
# 6/10 → 1/10). Codex 자문 결과 청크 임베딩은 그대로 두고, 파일 단위 synopsis를
...
def _doc_synopsis_candidates(vec: list[float], store: Store,
    ...
    for dh in doc_hits:
        ...
        # 본문이 숫자뿐인 파일을 리랭커가 다시 낮게 평가할 수 있다(Codex 리뷰 지적).
        for h in extra:
            if synopsis:
...
def _rerank_text(h: Hit) -> str:
    ...
    오해를 준다는 Codex 리뷰 지적을 반영해 "폴더"로 표기한다).
    ...
    # (docsyn 에서 이미 겪은 문제 — Codex 리뷰 지적). 검색 보조 정보임을 명시해
...
def _retrieve_canonical(query: str, catalog: ManifestCatalog, record: CanonicalRecord,
    ...
    # 파일(예: "연구실 소개.pptx")이 조용히 전체 반환될 수 있다(Codex 리뷰, 2차
    # 지적). record 에 미리 등록해둔 filename_hints 가 실제로 그 파일명에 맞아
...
# 하나하나가 논문 프로젝트 하나("Agronomy(published／Feb 2024)" 등)와 대응한다.
...
# 없다(Codex 리뷰 지적). 대신 §28과 같은 방식으로 각 폴더의 최신 mod_time을
...
def _retrieve_subfolder_listing(query: str, record: CanonicalRecord, store: Store,
    ...
    # 소리 없이 무시된다 — Codex 리뷰 지적). allowed_roots 로 범위가 좁혀졌으면
...
# ── 미인덱싱 파일 위치 안내 ────────────────────────────────────────────
# 실측: "예시 서비스 서비스"를 물으면 "(연예시 서비스트)예시 서비스_서비스소개서.pdf"가 스캔본
...
# 잡음이 된다(Codex 리뷰 지적) — 구체적인 단어(예: "예시 서비스")가 걸렸을 때만
...
# (Codex 리뷰 지적 — "raw error를 그대로 노출하지 말 것"을 보수적으로 반영).
_SKIP_REASON_MAP = (
